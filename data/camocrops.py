...
VOC_CLASSES = ('ampalaya', 'atis', 'avocado', 'bell_pepper', 'calamansi', 'cucumber', 'dalandan', 'french_bean', 'green_chili', 'green_mango', 'guava', 'guyabano', 'okra', 'pea', 'sayote')
...
class CAMOCROPS(Dataset):
    ...
    def __init__(self,
                 ...
                 keep_difficult=False):
        """Class constructor for CAMOCROPS
        ...
        self.annotation_path = osp.join(self.data_path, "Annotations", "{}.xml") # annotaion files

        self.image_path = osp.join(self.data_path, "JPegImages", "{}.jpg") # images files

        self.text_path = osp.join(self.data_path, "ImageSets", f"{self.mode}.txt") # splits text files

        self.ids = []
        if osp.exists(self.text_path):
            with open(self.text_path, "r") as f:
                self.ids = [line.strip() for line in f]  # Store only image filenames
        else:
            raise FileNotFoundError(f"Split file {self.text_path} not found.")
    ...
    def pull_item(self,
        ...
        if not osp.exists(target_path):
            raise FileNotFoundError(f"Annotation file not found: {target_path}")
...
def save_results(all_boxes,
    ...
    detections_dict = {}  # Store detections for JSON output

    # for each class
    for class_i, class_name in enumerate(VOC_CLASSES):

        text = 'Writing {:s} CAMOCROPS results file'.format(class_name)
                ...
                if len(detections) != 0:
                    for k in range(detections.shape[0]):
                        ...
                        output = output.format(image_id,
                                               ...
                                               detections[k, 0] + 1,  # xmin
                                               detections[k, 1] + 1,  # ymin
                                               detections[k, 2] + 1,  # xmax
                                               detections[k, 3] + 1)  # ymax

                        f.write(output)

                        # Store in JSON dictionary
                        if image_id not in detections_dict:
                            detections_dict[image_id] = []

                        detections_dict[image_id].append({
                            "class": class_name,
                            "confidence": float(detections[k, -1]),
                            "bbox": [float(detections[k, 0] + 1),
                                     float(detections[k, 1] + 1),
                                     float(detections[k, 2] + 1),
                                     float(detections[k, 3] + 1)]
                        })

    # Save JSON file
    json_filename = osp.join(results_path, "detections.json")
    with open(json_filename, "w") as json_file:
        json.dump(detections_dict, json_file, indent=4)

    print(f"Detections saved in {json_filename}")
...
def camocrops_eval(detection_path,
             dataset,
             annotation_path,
             list_path,
             class_name,
    ...
    if not osp.isfile(cache_file):
        ...
        for i, image_name in enumerate(image_names):
            temp_path = osp.join(dataset.data_path, "Annotations", f"{image_name}.xml")
    ...
    for image_name in image_names:
        ...
        difficult = np.array([x['difficult'] for x in target]).astype(np.bool_)
...
def do_python_eval(results_path,
    ...
    list_path = osp.join(dataset.data_path, "ImageSets", f"{mode}.txt")
