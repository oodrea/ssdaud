...
class SSDEfficientNet(nn.Module):
    ...
    def init_weights(self, model_save_path=None, basenet=None, use_gpu=True):
        """
        Initializes the EfficientNet backbone and SSD-specific layers.
        - If `basenet` is provided, it loads from a file (only if it's NOT vgg16_reducedfc.pth).
        - Otherwise, it loads EfficientNet-B0 ImageNet weights from torchvision.
        - Moves the model to GPU if `use_gpu=True`.
        """

        if basenet and "vgg16_reducedfc.pth" not in basenet:  
            weights_path = osp.join(model_save_path, basenet)
            print(f"Loading EfficientNet backbone weights from: {weights_path}")
            self.base.load_state_dict(torch.load(weights_path, map_location="cpu"), strict=False)
        else:
            print("Loading EfficientNet with ImageNet-1K pretrained weights.")
            self.base = efficientnet_b0(weights="IMAGENET1K_V1")

        # Determine device
        self.device = torch.device("cuda" if use_gpu else "cpu")
        self.to(self.device)  # Move the entire model to the correct device

        # Apply Xavier initialization for SSD-specific layers
        self.extras.apply(xavier_init)
        self.class_head.apply(xavier_init)
        self.loc_head.apply(xavier_init)

        print(f"SSD EfficientNet backbone initialized successfully on {self.device}.")
...
# Modified (11/03/2025 - 12:14am)
def get_extras(config, in_channels):
    layers = []
    print("Building extras layers:")
    i = 0
    while i < len(config):
        # Skip over markers if necessary
        if config[i] == 'S':
            print(f"Encountered 'S' marker at index {i}, skipping")
            i += 1
            continue

        # If the next element is a 'S' marker, then create a conv with stride=2
        if i + 1 < len(config) and config[i + 1] == 'S':
            ...
            print(f"Creating extra layer with downsampling: in_channels={in_channels} -> out_channels={out_channels} (stride=2)")
            layer = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=2, padding=1)
            layers.append(layer)
            in_channels = out_channels
            i += 3  # Skip the marker and the out_channels we just used
        else:
            out_channels = config[i]
            print(f"Creating extra layer: in_channels={in_channels} -> out_channels={out_channels} (stride=1)")
...
# Modified (11/03/2025 - 12:18am)
def multibox(config, base, extra_layers, class_count):
    class_layers = []
    loc_layers = []

    # Backbone heads: extract from layer 3 and layer 4
    efficientnet_layers = [3, 4]
    for k, v in enumerate(efficientnet_layers):
        in_channels = base.features[v][0].out_channels
    ...
    # Extra layers: use every layer (now we have 4 extra layers)
...
def build_extras(in_channels):
    extras = nn.ModuleList([
        # Extra source 2: from 19×19 to 10×10
        nn.Conv2d(in_channels, 256, kernel_size=3, stride=2, padding=1),
        # Extra source 3: from 10×10 to 5×5
        nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1),
        # Extra source 4: from 5×5 to 3×3 (using stride=2 may yield 3×3 if 5 rounds up to 3)
        nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1),
        # Extra source 5: from 3×3 to 1×1, use kernel_size=3, stride=1, no padding
        nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=0),
    ])
    return extras


def build_SSDEfficientNet(mode, new_size, anchors, class_count, scale_initial, scale_min, scale_max):
    base = efficientnet_b0(weights="IMAGENET1K_V1")

    extras = build_extras(in_channels=80)
    ...
    return SSDEfficientNet(mode=mode,
                           base=base,
                           extras=extras,
                           head=head,
                           anchors=None,
